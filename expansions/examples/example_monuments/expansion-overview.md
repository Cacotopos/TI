# Monuments Expansion Overview

## Metadata

- **Name**: Monuments
- **Version**: 3.1
- **Release Date**: 2026-06-01

## Summary

Generate a static expansion site for the Monuments expansion. The site should
follow the dark, card-first visual style of the existing card_diff report.

## Sections

1. **Overview** — A short description of the expansion.
2. **Factions** — Cards for each new or revised faction.

## Source Assets

- Images: `src/images`
- Icons: `src/icons`

## Agent Instructions

Use the shared templates in `expansions/templates/` to build the site. The site
must be self-contained (HTML, CSS, JS, images) and ready for S3 deployment.
